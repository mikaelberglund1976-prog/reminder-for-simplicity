...
type FormState = { name: string; category: string; date: string; recurrence: string; amount: string; currency: string; note: string; reminderDaysBefore: string; };
...
export default function EditReminderPage() {
  ...
  const [form, setForm] = useState<FormState>({ name: "", category: "SUBSCRIPTION", date: "", recurrence: "YEARLY", amount: "", currency: "SEK", note: "", reminderDaysBefore: "3" });
  ...
  async function fetchReminder() {
    try {
      ...
      setForm({ name: data.name, category: data.category, date: data.date ? data.date.slice(0, 10) : "", recurrence: data.recurrence, amount: data.amount != null ? String(data.amount) : "", currency: data.currency || "SEK", note: data.note || "", reminderDaysBefore: String(data.reminderDaysBefore ?? 3) });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/reminders/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    ...
    <div className="min-h-screen bg-[#F5F6FA]">
      <header className="bg-white border-b border-[#E8EDF4] px-5 py-4 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <Link href={`/dashboard/${id}`} className="text-[14px] font-medium text-[#8B90A4] hover:text-[#1A2340] transition-colors">← Back</Link>
          <span className="text-[#E8EDF4]">|</span>
          <h1 className="text-[16px] font-semibold text-[#1A2340]">Edit reminder</h1>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-5 py-6 pb-12">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="card">
            ...
            <input type="text" className="input" placeholder="e.g. Spotify, Netflix, Mom's birthday" value={form.name} onChange={(e) => set("name", e.target.value)} required />
          </div>
          ...
          <div className="flex gap-3">
            <Link href={`/dashboard/${id}`} className="btn-secondary flex-1 text-center">Cancel</Link>
            <button type="submit" className="btn-accent flex-1" disabled={saving}>{saving ? "Saving…" : "Save changes"}</button>
          </div>
        </form>
      </main>
    </div>
