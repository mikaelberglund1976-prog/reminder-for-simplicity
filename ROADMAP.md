...
**Uppdaterad igen:** 2026-07-28 – stor UX-genomgång byggd (se `TODO.md` punkt 19/20, `PRODUCT_SPEC.md` 4b.25–4b.29): bottenmenyn är nu anpassningsbar per person, kalendern har fått typfilter/månadslista/"+"-guide, Chores/Training är uppdelade i egna sektioner, inköpslistans "add item" är ombyggd med streckkodsskanning och butiksläge, och en `/privacy`-scaffold är på plats. Ett fynd som ändrar prioritet på en tidigare öppen punkt: kontosammanslagning (Google + lösenord) var redan till stor del byggd sedan tidigare, bara inte synlig som egen funktion – se 4b.29.
**Uppdaterad igen:** 2026-07-28 – Fas 1.5-listan kompletterad med de fyra punkterna ovan (bottenmeny, kalender, Chores/Training, kontosammanslagnings-fyndet). Även ett Vercel-byggfel hittat och fixat samma dag efter denna omgångs push (`useSearchParams` utan `Suspense` på `/dashboard/new` och `/dashboard/school`) – se `TODO.md` punkt 21.
...
- [x] **Anpassningsbar bottenmeny per person (2026-07-28):** nytt `User.bottomNavTabs`-fält. Calendar är den enda obligatoriska fliken; övriga 2–3 (Reminders/Shopping list/Wishlist/Chores/Training/School) väljs fritt i Profile → Preferences. Hamburgermenyn innehåller alltid alla sidor oavsett vad som är valt i bottenmenyn. Default: Reminders, Shopping list, School. Se `TODO.md` 19a/20, `PRODUCT_SPEC.md` 4b.10.
- [x] **Kalenderuppgraderingar (2026-07-28):** typfilter/färglegend (Reminders/Chores/Training/School, av/på per typ), en "Everything this month"-lista under dagspanelen, och en flytande "+"-knapp med en 2-stegs guide (typ → datum) som förifyller rätt formulär. Se `TODO.md` 19b/20, `PRODUCT_SPEC.md` 4b.25.
- [x] **Chores/Training uppdelning slutförd (2026-07-28):** Chores-sidan städad (barn-inloggningslänk och shopping-list-genväg borttagna, "This year"-statistik tillagd), Training fick en egen sida (`/dashboard/training`, mirror av School-mönstret) istället för att dela yta med Chores. Se `TODO.md` 19d/20, `PRODUCT_SPEC.md` 4b.26.
- [x] **Fynd: kontosammanslagning (Google + lösenord, samma email) var redan byggd (2026-07-28):** upptäckt vid genomgång av `auth.ts` inför 19g, inte nybyggt. Google-inloggning återanvänder redan befintligt `User` på matchande email (ingen dubblett), och hushålls-inbjudningars "flytta allt ur gammalt hushåll"-logik (`autoJoinPendingInvite`) är i praktiken redan den sammanslagningsmodell vi bestämde. Det enda som saknas är en bekräftelseskärm innan bytet — medvetet inte byggd denna omgång, kräver en egen fokuserad runda på `auth.ts` med riktig Google-inloggningstestning. Se `TODO.md` 19g/20, `PRODUCT_SPEC.md` 4b.29.
...
- [ ] ~~Gästprofiler utan inloggning~~ **Ersatt 2026-08-02:** Mikael beslutade motsatsen – samarbete ska kräva inloggning/konto, inte vara anonymt. Se `PRODUCT_SPEC.md` 4b.31 (ny "Guest"-roll, kräver konto) och `LAUNCH_CHECKLIST.md` Fas E.
...
- [ ] **Belöningar kopplat till Sysslor** *(Best4Family-analys 2026-07-28 – utökning av befintlig `ChoreStatus`-godkännandeflow, se `TODO.md` punkt 9)* — **beslutat 2026-08-02: poäng/stjärnor (virtuellt), inte riktiga pengar/belöningar i v1.** Se `PRODUCT_SPEC.md` 4b.3 för motivering. Inte längre en öppen fråga.
...
- [ ] Mobilapp – **inte symmetriskt mellan plattformarna** (research 2026-08-02, se `APP_STORE_READINESS.md`): Android/Google Play är lågt jobb ovanpå den PWA-grund som redan finns (`manifest.json`/`sw.js`), men iOS/Apple kräver ett eget hybrid-utvecklingsprojekt (t.ex. Capacitor, inte React Native rakt av) eftersom Apple avvisar rena webview-wrappers av en befintlig webbplats. Kontoradering (fortfarande bara en UI-shell) blir en hård Apple-blockerare, inte bara ett GDPR-önskemål – se `TODO.md` punkt 25.
